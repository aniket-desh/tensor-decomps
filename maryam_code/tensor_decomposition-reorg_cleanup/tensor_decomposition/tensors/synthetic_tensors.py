...
def Generate_sample_3D_GZ_Euler_Maruyama(n):
    # Given constants
    #n = 8
    d = n**3
    h = 1 / (n + 1)
    ...
    for iteration in range(60):
        ...
        # Creating samples for 3D Ginzburg Landau
        for _ in range(N):
        ...
        # print(U.shape)
        #print(numpy.length(all_samples))
...
def phi(i, x):
    ...
    Args:
    - i (int): Index of the Fourier mode.
    - x (float or np.ndarray): Input value(s).

    Returns:
    - np.ndarray: Value of the Fourier basis function at x.
    """
    if i == 0:
        return np.ones_like(x) / np.sqrt(2)  # Constant term
    elif i % 2 == 1:  # Odd index -> sine term
        return np.sin(np.pi * x * (i // 2 + 1))
    else:  # Even index -> cosine term
        return np.cos(np.pi * x * (i // 2 ))

def Generate_C_core(tenpy, dims, args.s, args.k, args.epsilon, args.seed):
    N = len(dims)
    U = []
    all_samples = Generate_sample_3D_GZ_Euler_Maruyama(s)
    print(all_samples.shape)
    interior_samples = all_samples[:, 1:-1, 1:-1, 1:-1]
    # Find batches containing NaN values
    print(interior_samples.shape)
    invalid_batches = np.any(np.isnan(interior_samples), axis=(1,2,3))
    
    # Filter out those batches
    filtered_samples = interior_samples[~invalid_batches]
    
    print(f"Original number of batches: {filtered_samples.shape[0]}")
    print(f"Number of batches after filtering: {filtered_samples.shape[0]}")
    
    # Normalize range to [-1, 1]
    filtered_samples = filtered_samples / 2.5
    print(filtered_samples.shape)
    num_samples = filtered_samples.shape[0]
    n = filtered_samples.shape[1]
    
    # Generate grid points in [-1, 1] for each dimension
    grid_points = np.linspace(-1, 1, n)
    
    # Initialize tensor C
    C = np.zeros((n , n , n))
    # Initialize factor matrices
    # U[0] = np.zeros((num_samples, n))
    # phi_j = np.zeros((num_samples, n))
    # phi_k = np.zeros((num_samples, n))
    R = num_samples
    # Compute C_ijk
    for i in range(n):
        for j in range(n):
            for k in range(n):
                # Evaluate phi_i(x), phi_j(y), phi_k(z) at all grid points
                phi_i_vals = phi(i, grid_points)
                phi_j_vals = phi(j, grid_points)
                phi_k_vals = phi(k, grid_points)
    
                # Sum over all samples
                sum_value = 0
                for l in range(num_samples):
                    for xi in range(n):   # Loop over x-coordinates
                        for yi in range(n):   # Loop over y-coordinates
                            for zi in range(n):   # Loop over z-coordinates
                                sample_value = interior_samples[l, xi, yi, zi]
                                sum_value += (
                                    phi_i_vals[xi] * phi_j_vals[yi] * phi_k_vals[zi] * sample_value
                                )
                                        # Store factor matrix values for this sample
                                U[0][xi,l] = phi_i_vals
                                U[1][yi,l] = phi_j_vals
                                U[2][zi,l] = phi_k_vals
                # Normalize by the number of samples
                C[i, j, k] = sum_value / num_samples
    print("Tensor C computed with shape:", C.shape)
    ...
    return C, T_noise, covs_empirical, covs_pinv_empirical, M_empirical_pinv
# C = Generate_C_core(8)   
