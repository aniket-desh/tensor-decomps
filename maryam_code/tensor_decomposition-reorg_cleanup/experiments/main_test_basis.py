...
#####################################################################################
    ...
    for run in range(args.num_runs):
        ...
        degree_spatial = [degree_x, degree_y, degree_z]
        print(degree_v)
        print(degree_spatial)
        e_ana = []
        e_amdm = []
        e_als = []
        for basis_functions_spatial in type_basis:  
            basis_functions_velocity = basis_functions_spatial
            print(basis_functions_velocity)
            phi_basis_x = []
            knots_x = []
            M1 = []
            if basis_functions_spatial == "B-spline":
                degree_x = 2
                degree_y = degree_x
                degree_vx = degree_x
                degree_vy = degree_x
                ...
                #     print(M1)
                # print(phi_basis_x)
            elif basis_functions_spatial == "Chebyshev":
                for i in range(D):
                    ...
                    M1.append(M1_i)
            elif basis_functions_spatial == "Legendre":
                for i in range(D):
                    ...
                    M1_i = compute_Mi(phi_basis, degree_spatial[i], symbol_spatial[i], interval_x[i][0], interval_x[i][1])
                    M1.append(M1_i)    
            elif basis_functions_spatial == "Fourier":
                for i in range(D):
                ...
                #     print(M1)
                # print(phi_basis_x)
            elif basis_functions_spatial == "Gaussian":
                for i in range(D):
            ...
            knots_v = []
            M2 = []
            if basis_functions_velocity == "B-spline":
                for i in range(D):
                    ...
                    M2.append(M2_i)   
            elif basis_functions_velocity == "Chebyshev":
                for i in range(D):
        ...
        #########################################################################
                    ...
                    for R in ranks:
                        ...
                        error1 = reconstructed_array - reconstructed_array_C_r
                        error2 = reconstructed_array - reconstructed_array_C_r_als
                        errors1_L2 = (np.linalg.norm(reconstructed_array.astype(np.float64) - reconstructed_array_C_r.astype(np.float64)))/np.sqrt(len(reconstructed_array.flatten()))
                        errors2_L2 = (np.linalg.norm(reconstructed_array .astype(np.float64) - reconstructed_array_C_r_als.astype(np.float64)))/np.sqrt(len(reconstructed_array.flatten()))
                        e1.append(errors1_L2)
                        e2.append(errors2_L2)
        ...
        ##########################################################################            
            ...
            e_als.append(e2)
            if basis_functions_velocity == "B-spline":
                e_bspline.append(err)
                # print(1)
                # print(e_bspline)
                e_amdm_bspline.append(e1)
                e_als_bspline.append(e2)
            elif basis_functions_velocity == "Fourier":
                e_fourier.append(err)
                e_amdm_fourier.append(e1)
                e_als_fourier.append(e2)
            elif basis_functions_velocity == "Gaussian":
                e_gaussian.append(err)
                e_amdm_gaussian.append(e1)
                e_als_gaussian.append(e2)
        # print(e_ana)
        # print(e_amdm)
        # print(e_als)
        #print(e_bspline)
        # print(e_fourier)
        # print(e_gaussian)
        # print(e_amdm_bspline)
        # print(e_amdm_fourier)
        # print(e_amdm_gaussian)
        # print(e_als_bspline)
        # print(e_als_fourier)
        # print(e_als_gaussian)
    e_bspline_best = np.min(e_bspline, axis = 0) 
    e_fourier_best = np.min(e_fourier, axis = 0)
    e_gaussian_best = np.min(e_gaussian, axis = 0)
    e_amdm_bspline_best = np.min(e_amdm_bspline, axis = 0) 
    e_amdm_fourier_best = np.min(e_amdm_fourier, axis = 0)
    e_amdm_gaussian_best = np.min(e_amdm_gaussian, axis = 0)
    e_als_bspline_best = np.min(e_als_bspline, axis = 0) 
    e_als_fourier_best = np.min(e_als_fourier, axis = 0)
    e_als_gaussian_best = np.min(e_als_gaussian, axis = 0)
    plt.figure(figsize=(10, 5))
    plt.subplot(2,3,1)
    plt.plot(sample_sizes, e_bspline_best, label='B-spline')
    plt.plot(sample_sizes, e_fourier_best, label='Fourier')
    plt.plot(sample_sizes, e_gaussian_best, label='Gaussian')
    #plt.plot(sample_sizes, err_L2_rho, label='L2 Error reconstructed particle density')
    plt.xlabel('Number of Samples')
    plt.ylabel('Error')
    plt.title('Error Convergence (reconstructed Maxwellian)')
    plt.legend()
    plt.xscale('log')
    plt.yscale('log')

    plt.subplot(2, 3, 2)
    plt.plot(sample_sizes, e_amdm_bspline_best, label='B-spline')
    plt.plot(sample_sizes, e_amdm_fourier_best, label='Fourier')
    plt.plot(sample_sizes, e_amdm_gaussian_best, label='Gaussian')
    #plt.plot(sample_sizes, err_L2_rho, label='L2 Error reconstructed particle density')
    plt.xlabel('Number of Samples')
    plt.ylabel('Error')
    plt.title('Error Convergence (AMDM)')
    plt.legend()
    plt.xscale('log')
    plt.yscale('log')

    plt.subplot(2, 3, 3)
    plt.plot(sample_sizes, e_als_bspline_best, label='B-spline')
    plt.plot(sample_sizes, e_als_fourier_best, label='Fourier')
    plt.plot(sample_sizes, e_als_gaussian_best, label='Gaussian')
    #plt.plot(sample_sizes, err_L2_rho, label='L2 Error reconstructed particle density')
    plt.xlabel('Number of Samples')
    plt.ylabel('Error')
    plt.title('Error Convergence(ALS)')
    plt.legend()
    plt.xscale('log')
    plt.yscale('log')
    # plt.subplot(1, 2, 2)
    ...
    # plt.legend()
    # plt.xscale('log')
    e_bspline_mean = np.mean(e_bspline, axis=0)
    #print(e_bspline_mean)
    e_bspline_std = np.std(e_bspline, axis=0)
    e_fourier_mean = np.mean(e_fourier, axis=0)
    e_fourier_std = np.std(e_fourier, axis=0)
    e_gaussian_mean = np.mean(e_gaussian, axis=0)
    e_gaussian_std = np.std(e_gaussian, axis=0)
        ###
    e_amdm_bspline_mean = np.mean(e_amdm_bspline, axis=0)
    e_amdm_bspline_std = np.std(e_amdm_bspline, axis=0)
    e_amdm_fourier_mean = np.mean(e_amdm_fourier, axis=0)
    e_amdm_fourier_std = np.std(e_amdm_fourier, axis=0)
    e_amdm_gaussian_mean = np.mean(e_amdm_gaussian, axis=0)
    e_amdm_gaussian_std = np.std(e_amdm_gaussian, axis=0)
        ###
    e_als_bspline_mean = np.mean(e_als_bspline, axis=0)
    e_als_bspline_std = np.std(e_als_bspline, axis=0)
    e_als_fourier_mean = np.mean(e_als_fourier, axis=0)
    e_als_fourier_std = np.std(e_als_fourier, axis=0)
    e_als_gaussian_mean = np.mean(e_als_gaussian, axis=0)
    e_als_gaussian_std = np.std(e_als_gaussian, axis=0)
    
    plt.subplot(2, 3, 4)
    iterations_err_bar = []
    iterations_err_bar.append(len(e_bspline_mean))
    iterations_err_bar.append(len(e_fourier_mean))
    iterations_err_bar.append(len(e_gaussian_mean))
    plt.errorbar(sample_sizes, e_bspline_mean, yerr=e_bspline_std, fmt='-o', capsize=5,
                     label=f"B-spline", color='blue')
    plt.errorbar(sample_sizes, e_fourier_mean, yerr=e_fourier_std, fmt='-*', capsize=5,
                     label=f"Fouier", color='orange')
    plt.errorbar(sample_sizes, e_gaussian_mean, yerr=e_gaussian_std, fmt='-.', capsize=5,
                     label=f"Gaussian", color='green')
    # plt.title("Residual vs Iterations with Error Bars")
        # plt.xlabel("iterations")
        # plt.ylabel("residual")
        # plt.legend()
        # plt.grid(True)
    #plt.xlim(left=min(sample_sizes), right=max(sample_sizes))
    plt.xscale('log')
    plt.yscale('log')
    plt.grid(linestyle='--')
    ...
    plt.subplot(2, 3, 5)
    iterations_err_bar = []
    iterations_err_bar.append(len(e_amdm_bspline_mean))
    iterations_err_bar.append(len(e_amdm_fourier_mean))
    iterations_err_bar.append(len(e_amdm_gaussian_mean))
    plt.errorbar(sample_sizes, e_amdm_bspline_mean, yerr=e_amdm_bspline_std, fmt='-o', capsize=5,
                     label=f"B-spline", color='blue')
    plt.errorbar(sample_sizes, e_amdm_fourier_mean, yerr=e_amdm_fourier_std, fmt='-*', capsize=5,
                     label=f"Fourier", color='orange')
    plt.errorbar(sample_sizes, e_amdm_gaussian_mean, yerr=e_amdm_gaussian_std, fmt='-.', capsize=5,
                     label=f"Gaussian", color='green')
    # plt.title("Residual vs Iterations with Error Bars")
        # plt.xlabel("iterations")
        # plt.ylabel("residual")
    ...
    iterations_err_bar.append(len(e_fourier_mean))
    iterations_err_bar.append(len(e_gaussian_mean))
    plt.errorbar(sample_sizes, e_als_bspline_mean, yerr=e_als_bspline_std, fmt='-o', capsize=5,
                     label=f"B-spline", color='blue')
    plt.errorbar(sample_sizes, e_als_fourier_mean, yerr=e_als_fourier_std, fmt='-*', capsize=5,
                     label=f"Fouier", color='orange')
    plt.errorbar(sample_sizes, e_als_gaussian_mean, yerr=e_als_gaussian_std, fmt='-.', capsize=5,
                     label=f"Gaussian", color='green')
    # plt.title("Residual vs Iterations with Error Bars")
        # plt.xlabel("iterations")
        # plt.ylabel("residual")
